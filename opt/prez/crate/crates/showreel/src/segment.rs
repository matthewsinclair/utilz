...
pub const SHAPES: &[Shape] = &[
  ...
  Shape {
    name: "card",
    fields: &["headline", "sub", "bg"],
  },
  Shape {
    name: "statement",
    fields: &["kicker", "headline", "body", "bg"],
  },
  Shape {
    name: "faq",
    fields: &["headline", "items", "bg"],
  },
  Shape {
    name: "atwork",
    fields: &["kicker", "name", "strap", "caption", "qr", "bg"],
  },
  Shape {
    name: "strapline",
    fields: &["mark", "lines", "bg"],
  },
  Shape {
    name: "points",
    fields: &["headline", "body", "points", "bg"],
  },
  Shape {
    name: "venue",
    fields: &["image", "kicker", "headline", "at", "city", "bg"],
  },
  Shape {
    name: "wordmark",
    fields: &["top", "mid", "bottom", "bg"],
  },
  Shape {
    name: "socials",
    fields: &["layout", "headline", "bg", "bgs"],
  },
  Shape {
    name: "logo",
    fields: &["file"],
...
pub fn pace(name: Option<&str>) -> Result<&'static Pace, Failure> {
  ...
  PACES.iter().find(|p| p.name == want).ok_or_else(|| {
    Failure::new(
      ...
      format!(
        "one of: {}",
        PACES.iter().map(|p| p.name).collect::<Vec<_>>().join(", ")
      ),
    )
  })
}
...
mod tests {
  ...
  fn a_pace_preset_carries_every_rendering_default() {
    ...
    assert_eq!(
      (p.transition, p.motion, p.fit),
      ("dissolve", "kenburns", "cover")
    );
    ...
    assert_eq!(
      (p.transition, p.motion, p.fit),
      ("dissolve", "drift", "cover")
    );

    assert_eq!(
      pace(None).unwrap().name,
      DEFAULT_PACE,
      "no pace named takes the default"
    );
    ...
    for p in PACES {
      assert!(
        TRANSITIONS.contains(&p.transition),
        "{}: {}",
        p.name,
        p.transition
      );
  ...
  fn every_rendering_value_the_live_reel_states_is_in_its_vocabulary() {
      ...
      for (field, allowed) in [
          ...
          .and_then(|v| v.as_str())
        {
          value(field, v, allowed, "live reel").unwrap_or_else(|e| {
            panic!(
              "the live reel states a value this port refuses: {}",
              e.message
            )
          });
          checked += 1;
    ...
    assert!(
      checked > 0,
      "the live reel stated no rendering values at all; test proved nothing"
    );
  }
  ...
  fn every_shape_the_reference_has_is_declared_here() {
    assert_eq!(
      SHAPES.len(),
      12,
      "eleven branches plus the gallery fallthrough"
    );
    for want in [
      "crawl",
      "card",
    ...
    assert_eq!(
      seen.len(),
      SHAPES.len(),
      "a duplicated shape name would shadow silently"
    );
  }
  ...
  fn a_mistyped_segment_type_is_refused_and_the_valid_set_is_given() {
    ...
    assert!(
      e.message.contains("crwal"),
      "names what was written: {}",
      e.message
    );
    let remedy = e.remedy.unwrap();
    assert!(
      remedy.contains("crawl"),
      "offers the intended type: {remedy}"
    );
    assert!(
      remedy.contains("gallery"),
      "and names what it would silently become: {remedy}"
    );
  }
  ...
  fn a_key_valid_for_a_different_type_is_still_refused() {
    ...
    assert!(
      e.message.contains("type 'crawl' has no key 'bg'"),
      "{}",
      e.message
    );
    assert!(
      e.remedy.unwrap().contains("source"),
      "gives crawl's own fields"
    );
  ...
  fn every_segment_of_the_live_reel_validates() {
    ...
    let reel: Just = serde_yaml::from_str(include_str!("../fixtures/45h.showreel.yaml")).unwrap();
