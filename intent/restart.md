---
verblock: "11 Sep 2026:v1.9: matts - EOW globalfold: 2.8.0, ST0017 closed, prez showreel, devbin gates wired"
---
...
## Key Context (as of 11 Sep 2026)

- **Framework at `2.8.0` with 15 utilities**, dispatched by `utilz` on top of them (`bin/` holds those 15 symlinks plus `utilz` itself and the vendored `devbin`). Tags drop the `v` from `2.7.0` on, deliberately. The next release carries ST0017. **Releases, tags and pushes are hv's**; the 9 Sep CI-credits freeze on `upstream` is lifted.
- **`prez showreel` (ST0017, CLOSED 11 Sep) compiles a directory of pictures and a YAML config into one self-contained, self-advancing HTML file.** `opt/prez/crate` is now a Cargo WORKSPACE: the root package is prez, `crates/artifact` is the theme/inlining/base64 code prez and showreel share, `crates/showreel` is the pipeline. It has `check` and `build` only -- **no `init` or `qr`**, so a reel's masters and QR codes still come from the Python prototype in the Snorkeltoast repo; porting them is the deferred follow-on (was WP-04). A reel naming a non-built-in theme needs `SHOWREEL_THEME_PATH`. The 45h reel on snorkeltoast.com is built by it. The thread is dehydrated: `intent st hydrate ST0017` brings back its design and contract.
...
- **`dvb test all` and `dvb check all` are wired to this project's layout** in `bin/.devbin/config.yaml` (11 Sep): cargo gates use the crate's `--manifest-path` with `--workspace`, and shell runs `bats opt/*/test`. **The `fmt rust` arm is disabled on purpose**: the crate is hand-formatted at 2-space and has never been through rustfmt, so `fmt rust` would rewrite all of it. Adopting rustfmt is on `wip.md`.
...
- **`intent/issues/OPEN/` is empty BY POLICY and always will be, so do not read it as "no open defects".** `intent/.intentfiles` declares that only OPEN THREADS get a realised form on disk; no issue is ever written there, and the files under `intent/issues/CLOSED/` are v2 artefacts predating that policy. Use `intent issues list`. Open: `0016` (`utilz install` does not consult the CI gates). An empty directory confirms a false answer where a missing one would announce itself.
...
The Utilz **framework** version is `VERSION` (single source of truth; read it, do not copy the number here). Per-utility versions in `opt/<name>/<name>.yaml` move independently. The **`intent` tooling** version (3.0.0) is a third thing; do not conflate them.
...
- `utilz test prez` -- drives three sources: the whole crate workspace's cargo tests, the shim BATS, and `crate/test/acceptance.sh --strict`. The acceptance suite needs a real browser; under `PREZ_TEST_BROWSER=/nonexistent` it takes the browserless path and says why it refused.
