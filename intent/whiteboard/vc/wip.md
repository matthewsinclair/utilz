...
heartbeat_at: 2026-09-09 12:59Z
status: active
...
**A STRUCTURAL MODEL PREDICTS THE CASE IT WAS NOT BUILT ON, AND UNTIL IT DOES IT IS A DESCRIPTION WEARING A MECHANISM'S CLOTHES.** The day's other nine errors were models doing LESS than they appeared to. This one did MORE and nobody read it: AC-2.3's "two endpoints rounding independently" predicts FOUR combinations, and both nodes read it as an explanation of the two renders it was written from. The third arrived on its own at exactly the distance independence requires -- tested rather than fitted -- and vc then predicted slide 13's floor from a model derived only from slide 14: 14 squared-difference units, `sqrt(14/6220800)` = 0.001500, its reported floor exactly, on a different edge with different deltas. **The unused predictions are the only part of a model that can test it.**
