...
- **A CLAIM ABOUT A FILE SOMEBODY ELSE IS EDITING HAS A SHELF LIFE MEASURED IN MINUTES.** vc and I wrote confident, correct-looking analyses of `strip_reason` twenty minutes apart and **both were against a version snorkeltoast had already replaced** -- mine argued their prediction was wrong using code that no longer existed. Neither analysis was sloppy; both were sound against what we had read. It is snorkeltoast's own rule with people in place of a stack trace: **do not edit a script while a long run of it is in flight, because the traceback resolves against the new file.** RE-READ IN THE SAME COMMAND THAT WRITES THE CLAIM -- the harness's mtime was ONE MINUTE before my read -- and **CITE BY TOKEN, NEVER BY LINE NUMBER**: the three call sites I cited at 07:5xZ had all moved by 08:29Z.
