...
- **THE VACUOUS ASSERTION IS THE ONE THAT LOOKS MOST RIGOROUS.** `assert!(s.contains(&format!("embed={}", TARGET)))` reads as _sourcing_ the value; `assert!(s.contains("embed=1920"))` reads as lazy. **It is the other way round.** The first moves when the constant moves, so it agrees with itself by construction and can never fail -- and I wrote it into the one test whose stated job was proving the stamp FOLLOWS its constants. Caught by the red-proof and by nothing else. **When a test's subject is "X derives from Y", the assertion must name X's VALUE, never Y.**
