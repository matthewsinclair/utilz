...
heartbeat_at: 2026-09-09 18:11Z
status: active
...
- **"IS THIS A PARITY FIX OR A DELIBERATE DIVERGENCE" IS ONE COMMAND, AND THE PORT CANNOT ANSWER IT.** A check that was missing and is now present looks IDENTICAL either way; only the reference distinguishes them. I framed `fit: cvoer` alongside the mistyped-`type:` defect -- but `type: crwal` is a REFERENCE defect section 5 rules a fix for, and `fit:` validation the reference already HAS at `showreel:828`. vc nearly filed the result as a fourth `PORT_EXPECTATIONS` row, which would have recorded a parity fix as an intended difference. **I had `:828` on screen in the same session before writing the sentence.** Ask _does the reference do this too_ BEFORE claiming a gap.
- **A CONTROL THAT REFUSES FOR ITS OWN REASON TURNS EVERY RED INTO A CONFIRMATION.** vc's first fixture used `type: image`, not one of the twelve shapes, so **all five arms exited 2 including the baseline** -- and running only the three arms expected to refuse would have read as three confirmations over a fixture that was invalid throughout. **The exact mirror of the empty population**: there everything passes, here everything refuses, and in both the treatment and the control return the same answer. **My own tests are refusal-shaped, so the second GREEN carries as much as the first** -- it is what proves the reds came from the injection rather than from drift.
- **HIGHLANDER'S CHECK IS THE WRONG SHAPE FOR A SUBSET DUPLICATE, AND THIS ESTATE HAS NO CHECK THAT IS THE RIGHT ONE.** A sweep compares two homes and finds the present fields agree; **the absent ones are not there to disagree.** Measured on mine: `main.rs` carried 2 of the pace table's 5 fields and both were CORRECT, which is exactly why it survived. Nothing consumed the missing three, so it was a SCHEDULED defect rather than a latent one. The question is not _do the two homes agree_ but **does the second carry every FIELD of the first.**
