...
heartbeat_at: 2026-09-09 19:50Z
status: active
focus: "ST0017/WP-03. Contract 42/51, WP-03 9/12 -- vc closed AC-3.3 and AC-3.8 while I was folded. The reference's OWN plan reaches 23 slides on the live config, so 23-vs-22 is settled as config-vs-008. EVERY JSON-FREE PIECE OF WP-03 IS BUILT. BLOCKED on hv for serde_json, which gates the payload and AC-3.6's satisfaction. Taking sequencing from vc until hv returns."
claims: [ST0017]
---
...
- **DATA-URI IS IN (`159f544`), 219 tests.** `embed()` over `normalise()`: the reference's `data_uri` does its own resize AND its own encoder choice, which is how it came to disagree with `normalise_image`. Here one function decides and one says how big, so **the two passes cannot drift without somebody editing the shared one.**
- **THE TEMPLATE (`cb8f2bf`), THE PLAN (`0bb78e0`), THE DELIVERY NAMING (`01d3469`).** 247 tests. `player.html` is ONE LINE from the reference; `check` now reports **23 slides / 14 assets**, the same two numbers the reference's `plan()` gives on the same config.
- **BLOCKED, AND IT IS THE ONLY THING LEFT IN WP-03: `serde_json` NEEDS hv.** The manifest says a crate addition needs hv's sign-off named in the commit. Net cost measured against THIS tree: **2 packages, 79 to 81** -- `itoa`, `memchr`, `serde_core` are already here and only `serde_json` and `zmij` are not. **I nearly reported +4 by reading the crate's own tree instead of the difference.** vc raised its priority on better grounds than mine: it gates **AC-3.6's satisfaction**, not a dependency count.
- **WHAT THE PAYLOAD STILL OWES:** `build_socials` (with the stale-QR warning, which is AC-4.2's text), `build_bug` (**and its admission -- vc's prediction, `plan.rs` joins `bug.file` unclassified while four `Requires::Image` sites refuse**), the slide rows, the limits block, and the `build` verb that writes `_out/` and calls report + prune.
- **THE BRAND DROP IS NOT COSMETIC AND I HAD THE WRONG REASON FOR IT.** `player.html:628` is `add("Producer", REEL.producer || "Snorkeltoast")`, pulled with `include_str!`, so **a verbatim pull takes the H3 strings zero to one.** hv withdrew AC-3.7 saying nothing gates on the literal; nothing in the CONTRACT does, and my own gate now does. Baseline before the pull, control at 3: `popupart` 0, `POP^UP^ART` 0, `Snorkeltoast` 0, `showreel.yaml` 3. **45h sets `producer:` itself, so the drop changes no pixel and changes the binary.**
- **AC-3.6's RUNTIME LEG IS MINE AND REDUCES TO ONE PROPERTY:** the emitted payload's `limits.max_ease` **DERIVES** from `limits::MAX_EASE_MS`, never a restated literal. `player.html:561` is `const LIM = REEL.limits` -- **the cap is read off the payload, not a JS literal.** Red-control: flip the constant to 3000, the payload must follow. **Cite `showreel-harness:232` and `:1098`, NOT `shipped-max-ease`** -- vc's correction: that row retires itself the day Python is capped, so it is a citation with an expiry date.
...
**AND THE FOURTH, WHICH IS ABOUT THE INSTRUMENT ITSELF: A RED-PROOF HARNESS THAT DOES NOT CLEAN UP ON REFUSAL IS WORSE THAN ONE THAT NEVER REFUSES.** Mine wrote the injection, then refused on a post-check whose anchor had collided with an unrelated match arm -- **with the revert on the success path only.** The tree sat with `iso()` not validating dates at all while the console read `REFUSED`, **which is exactly what "nothing happened" looks like.** Caught only because the next thing I did was read the function. The revert belongs in a `finally`. **A reassuring failure message is the worst kind.**

**AND THE COUNT OF FIRING INJECTIONS IS NOT THE CONTROL -- EACH ONE FIRING IS.** vc's line, earned twice in one slice: `G` moved nothing because Rust's `Path` drops `CurDir` before my helper mattered, and `L` moved nothing because a zero-padded fixture makes lexical and numeric order coincide. **Four of five, and five of six, both read as good results.** A test that asserts nothing looks exactly like one that passes.

**AND THE THIRD, WHICH IS NOW THE MOST PRODUCTIVE QUESTION I HAVE: WHAT DOES 45h FAIL TO CONTAIN?** Four instances in one afternoon, each found by asking it rather than by suspicion. **(1) The bug embed** -- `bug.file` IS the logo segment's file, so the bug enters `used` through the slide walk and a build that dropped it entirely still counts 14. **(2) AC-3.4's artifact half** -- the reel carries NO opaque RGBA at all, both brand PNGs being genuinely transparent, so design.md 4.2's named instance ("the mascot and the wordmark, on every build") is refuted and the exemption list is EMPTY here. vc is re-measuring it by declared PNG colour type, which answers the stronger question with no pixel scan. **(3) The stale-QR warning** -- 45h's QRs match their config, so it cannot fire. **(4) The collapse arm** -- covered by three synthetic tests and by nothing on the reel. **A GREEN FROM 45h IS EVIDENCE ABOUT WHAT 45h CONTAINS, AND SILENCE IS WHAT A READER TAKES FOR COVERAGE.** The decisions were all right on their merits; the cost estimates came from reading the code instead of the pictures.
