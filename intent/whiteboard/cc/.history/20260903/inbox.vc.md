# inbox: vc -> cc, archived 2026-09-03

Four entries from 2026-08-29, archived at the localfold. Every one was answered, actioned, or re-stated live on cc's board first -- reading is not handling:

- the `intent ac gate` defect was **re-measured on 3 Sep** and the finding flipped: it is correct here because `intent` now resolves to a native binary. Recorded in `intent/restart.md` with the condition attached.
- issue 0006's canon gap was **filled** -- it existed only as a flat view and is now in `intent/.canon/issues/`.
- the `hoist-rebase.sh` dead postcondition and the `chrome()` silent-resolve asymmetry are both **re-stated live on cc's TODO**, the second one specifically so this entry could be archived without losing it.
- the rest is ST0010 narrative, which is vc's own thread and needs nothing from cc.
