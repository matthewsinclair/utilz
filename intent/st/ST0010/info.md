...
1. **AC13 determinism is RED.** Six of the seven built-in themes carry `@media (prefers-color-scheme:)` blocks that flip the whole `--gp-` palette, so one built `.html` renders light on one laptop and dark on another, silently. hv has ruled: flatten all six to a single palette, each keeping its designed look. The ruling is UNEXECUTED.
2. **`src/mermaid.rs:37-38` keys the diagram palette off `prefers-color-scheme` too.** An independent second violator of the same property, found by hv's eye at a demo and caught by no test.
...
`_tools` recorded twelve to thirteen instances over two days of the same failure: a check that reads plausibly and measures something adjacent to what it names. The operational form, in `_tools-vc`'s words, is to ask not "does this pass?" but what would have to be true for it to go RED, and whether that is the same thing the check is named after. If the answer needs an "and also", it is measuring something adjacent.
...
- **Every contrast figure predating `_tools` commit `ec3564a` is void.** `test/theme-legibility-probe.html:51` used `querySelector`, singular, so it measured the first text element on each slide -- always the heading, always full-strength foreground -- and called it worst-case. Honest worst-case numbers after the repair: 8bit 4.7, mono 4.9, steampunk 5.5, manuscript 5.7, simple 6.0, geodica 6.9, blueprint 7.6, contrast 14.2, against a 4.5 floor. Nothing unreadable ever shipped; the check simply could not fail.
