...
**D3 -- RESOLVED BY hv, 8 Sep: symmetry, and no `v` anywhere.** `utilz --version` prints `utilz:2.6.1`. The framework is not "part of" anything, so it is the **degenerate case of the same form** rather than a second format: the framework half is spelled identically alone and inside a pair, character for character on the part they share. hv's words were "we shouldn't have `v` in the version number anywhere, it's just noise", and `common.sh:189` was **the estate's only minter of one**.

**D3a -- lines two and three are NOT part of that ruling and are asserted, not assumed** (vc). The description stays, and so does the tree-provenance line (`source at <tree>` / `installed at <tree> (<sha>)`), which is held by **ST0014/AC12 on a CLOSED thread**: an installed utilz must be distinguishable from a source utilz at the prompt. An implementation that rewrote line one and tidied line three away would break a satisfied criterion on another thread, and nothing in AC01-AC08 would have caught it. **It earned that within the hour**: hv was shown the new pair form and read it as done, but it was the CHECKOUT answering -- the published install was still stale, and the provenance line was the one command that said which tree had replied.
