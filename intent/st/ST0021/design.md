...
- **Chrome's temporary files, its process singleton among them, go in the scratch directory too.** Headless Chrome makes a singleton directory (`.com.google.Chrome.XXXXXX`, holding `SingletonSocket` and `SingletonCookie`) in its temporary directory. Both `TMPDIR` and `MAC_CHROMIUM_TMPDIR` name the scratch directory, because they fail differently:
  - **macOS Chrome ignores `TMPDIR`** and reads `MAC_CHROMIUM_TMPDIR`. Before both were set, every recording that ended in a kill left a singleton in the system's temporary directory, and 523 were counted there after one day of testing.
  ...
  - Measured on macOS after the fix: a full `video.sh` run, which kills Chrome on three paths, added no singleton to the system's temporary directory (286 before and after), and AT27 finds the socket inside the recording's scratch directory.
