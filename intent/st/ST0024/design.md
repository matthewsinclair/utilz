...
**Portrait only, as 6.1 asks, and widescreen is untouched by construction**: the headline half of the fit does not run in a landscape window, so a 16:9 recording cannot move. Whether headlines should fit at 16:9 as well is a real question and not this thread's: at 16:9 a long word breaks mid-word today, which is ugly and inside the frame, so it is a look, not a defect. It goes to hv with the rest.
...
### D5b. The geometry, and where it comes from

The zone is four insets, as a share of the frame. Read 2026-09-22, the ranges published for a 1080x1920 frame:
...
One zone covers both platforms rather than one per platform, because the same recording is posted to both and a zone per platform would mean a recording per platform. The numbers are the wider of the two at each edge, short of Instagram's 35% bottom, which is its guidance for ADS with a call to action rather than for a posted reel. **They are a moving target**: the platforms change their UI several times a year, so the design's job is to make the geometry one line to edit, which it is.
...
- **A player probe** over CDP, as AT12's determinism probe already drives Chrome from a plain `.mjs`: at a 540x960 viewport, every fitted line's longest word is on one line inside its room and no line's ink crosses the safe box; a line too long at its floor wraps instead of leaving the frame; at 960x540 the headlines are untouched and a handle whose ink fits is untouched. The probe is also what holds the contract that every fitted selector carries a `--fit-floor`.
- **`test/video.sh`**, three recordings, each red first: a 40-capital handle at 16:9 has no glyph in the frame's outer 86 px and is still drawn (6.3); a one-word headline too wide for 540 px at portrait has no glyph in the outer 20 px and is still drawn (6.1); and the same reel recorded `--safe-zone social` at portrait has no ink in the zone's four bands on its type pages, where the recording without the zone does.
- **By hand, as evidence rather than as a test** (ST0023's precedent): the 16:9 fixtures record byte-identical, frame for frame, before and after.
- **vc reads** a 9:16 recording of Snorkeltoast 001 with the zone and without it, one frame per segment type.

## D8. Release

2.12.0, prez 2.4.0 (vc, on hv's "less worried about the release number"), the prez bump in its own commit before the cut (ST0019 D2). A CHANGELOG entry under the open section, and `--safe-zone` in `showreel`'s help.
