...
- **CI cancels a run that a newer push supersedes, and the macOS legs run on the TAG, not on the push** (issue 0046). A superseded commit's run concludes `cancelled`, which `tools/ci-state`, the core's CI query, reads as `failed`; the core reads CI after it pushes, so nothing may land on `main` between the release push and that read, which the hold above already guarantees. A release commit therefore carries two runs, the branch's without macOS and the tag's with it, and the query asks for one: read both before calling a release green.
